...
public class OIDCTokenVerification {
    ...
    public OidcPayload verifyIdToken(
        ...
        HeaderInfo headerInfo = extractFromHeader(idToken);
        OIDCPublicKey matchedKey = oidcPublicKeys.getMatchedKey(headerInfo.kid(), headerInfo.alg());
    ...
    private HeaderInfo extractFromHeader(String token) {
        try {
            String headerJson = new String(Base64.getDecoder().decode(token.split("\\.")[0]));
            Map<String, ?> header = objectMapper.readValue(headerJson, Map.class);
