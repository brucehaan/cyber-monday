...
2. 인증된 아이디, 적립금, "쿠폰", "주문내역" 메뉴를 확인한다
...
#### CASE 7-2 : 쿠폰 페이지에서 쿠폰 다운로드

1. 마이페이지 버튼을 통해 마이페이지로 들어간다
2. "쿠폰" 메뉴를 클릭해 쿠폰 페이지로 들어간다
3. "쿠폰 받기" 탭을 클릭한다
4. 사용자 정보를 기준으로 받을 수 있는 여러 쿠폰들이 나열된다
5. 원하는 쿠폰의 아이콘을 클릭해 쿠폰을 다운로드 받는다
6. 다운로드 받으면 "쿠폰을 성공적으로 받았습니다"라는 안내창이 보인다
   - 받은 쿠폰의 아이콘이 비활성화된다
7. "받은 쿠폰" 탭으로 가서 다운로드 받은 쿠폰을 확인한다

#### CASE 7-3 : 쿠폰 등록하기

1. 마이페이지 버튼을 통해 마이페이지로 들어간다
2. "쿠폰" 메뉴를 클릭해 쿠폰 페이지로 들어간다
3. 쿠폰 페이지에서 "쿠폰 등록" 탭을 클릭해 등록 페이지로 이동한다
4. 등록화면에서 본인이 전달받은 쿠폰 번호를 입력한다
   - 만약 쿠폰 번호가 다르면 쿠폰이 발급되지 않으며, "쿠폰 발급에 실패했습니다"라는 경고 메시지가 나온다
   - 만약 이미 등록한 쿠폰 번호라면 "이미 발급된 쿠폰입니다"라는 경고 메시지가 나온다
5. 쿠폰을 발급받는다
6. "받은 쿠폰" 탭으로 가서 다운로드 받은 쿠폰을 확인한다
...
![쿠폰페이지_쿠폰다운로드](flow-chart/쿠폰페이지_쿠폰다운로드.png)

#### CASE 7-3 : 쿠폰 등록하기

![쿠폰 등록하기](flow-chart/쿠폰등록하기.png)
